...
OI Judge 是一款 VS Code 扩展，用于本地 OI 风格样例评测，也面向 OI 出题辅助流程。它可以帮助选手快速编译、运行、比较样例，也可以帮助出题人管理题目、样例、Subtask、STD、Generator、Checker、对拍、测试点导出和评测报告。
...
- 在 `.vscode/.OIJudge/config.json` 初始化工作区题目配置。
- 创建题目时不要求立刻绑定源文件。
...
- 首版不会自动运行 STD，也不会自动生成数据。
- 在 Setter Mode 中编辑的样例名只是显示名/数据配置名，不会改变 `sample.id`、`sample.index`、真实输入/答案文件或 `outputs/sample-<index>/`。
- 出题数据配置保存在 `setter.dataCases` 中，预留给未来的数据生成器工作流使用。
- 后续计划包括生成器配置、样例编号选择，以及自动生成输入/答案。

### Testlib Checker

- OI Judge 支持首版按题目配置的 testlib 风格 Checker。
...
- 后续版本可能加入 `score-json`、`score-plain` 和部分分 checker 协议。
