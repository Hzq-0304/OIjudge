...
- Manage Workspace is available from the OI Judge view title bar or the Command Palette.
...
- You can switch the mode from the problem's Configuration group in the OI Judge sidebar. When `File IO` is selected, the same group shows entries for the input and output file names.
...
- Problem workflow: `Create Problem`, `Add Problem From Current File`, `Add Problem From File`, `Import Legacy Single Problem`, `Delete Problem`, `Add Program`, `Set Default Program`, `Run With Program`, `Bind Statement`, `Open Statement`, `Unbind Statement`.
